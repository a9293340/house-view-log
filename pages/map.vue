...
import { getMapPhoto, setMapNull, makeMarker, nearbyApi } from '@/utils/mapApi.js';
...
const { setNearByStation, setPickLatLng, setStates, setPickHousePoint } = useMapStore();
...
const getStationInformation = async station => {
  ...
  if (station.photos) info.photos = station.photos.map(el => getMapPhoto(el.photo_reference));
  if (station) {
    if (currentMark) setMapNull(currentMark);
    if (newPointMark) setMapNull(newPointMark);
    newPointMark = makeMarker(states, info.lat, info.lng, info.name, newHouseImg, 40);
...
const searchFindData = async () => {
  ...
  changeMarkerAndOpenInformationBlock(
    (await nearbyApi(location.lat, location.lng, 5, jud ? autoSearch.name : autoComplete.value))
      .results,
    location.lat,
    location.lng,
  );
...
const goToCenter = (lat = currentLatLng.lat, lng = currentLatLng.lng, zoom = 15) => {
...
const setHousePoints = async () => {
  ...
  for (let i = 0; i < housePoints.value.length; i++) {
    ...
    const idx = housePointsMarker.findIndex(x => x.place_id === hPoint.place_id);
  ...
  states.map.addListener('click', async event => {
    ...
    changeMarkerAndOpenInformationBlock(
      await mapNearByStation(
        ['公寓', '大樓', '大廈', '宅', '邸'].map(
          async el => (await nearbyApi(event.latLng.lat(), event.latLng.lng(), 5, el)).results,
        ),
      ),
      event.latLng.lat(),
      event.latLng.lng(),
...
onBeforeMount(async () => {
  ...
  console.log(await nearbyApi(-33.867, 151.1957, 500, 'cruise'));
});
</script>
...
<template>
  <div class="map">
    ...
    >
      ...
      <NearByStationList v-if="informationBlockStatus === 0" @station="getStationInformation" />
...
.map {
  ...
  .information-block-close {
    @apply -bottom-96 lg:bottom-0 lg:-right-128;
  }
}
</style>
