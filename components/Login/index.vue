...
const checkAmountCase = computed(() => /[a-zA-Z\d]{8,}/.test(user.value.password));
...
const summitLogin = () => {
  ...
  if (props.loginType === 'N') {
    ...
    if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/.test(user.value.password)) {
      setErrorLog({
        title: '輸入錯誤!',
        type: 'error',
        description: '密碼不符合規範',
...
<template>
  ...
  <el-input
    ...
    size="large"
    class="login-input"
    :placeholder="loginType === 'L' ? '請填入密碼' : '密碼須至少8碼並包含大小寫跟數字各至少一個'"
